...
######## Simulating Gaussian data for the Merged
sim_data_merged <- function(nstudies, ncoef, ntest, sampsize){
  ...
  cluster_means <- t(sapply(1:nstudies, function(i) {
    sample(c(runif(round(ncoef/2), -5, -0.5), runif(ncoef - round(ncoef/2), 0.5, 5)))
  }
  ))
  ...
  icoefs <- c(4, 1.8)
  numsample <- round(2500/ntrain)
  
  for (i in 1:nstudies){
    curcoefs <- sapply(coefs, function(x){runif(1, x - .25, x + .25)})[vars]
    if  (i == nstudies){ #test set
      data_i <- as.matrix(do.call(rbind, lapply(1:ntrain, 
                                                function(t) {mvrnorm(n = numsample, mu = cluster_means[t, ], Sigma = diag(nchoose))})))
    }
    else{ #training set
      data_i <- mvrnorm(n = numsample, mu = cluster_means[i, ], Sigma = diag(ncoef))
    }
...
######## Simulating Gaussian data for the clusters
sim_data_cluster <- function(nstudies, ncoef, ntest, sampsize){
  ...
  for (i in 1:nstudies){
    ...
    if (i == nstudies){
      ...
      data_i <- mvrnorm(n = numsample, mu = cluster_means[1, ], Sigma = diag(ncoef))
    }
    y <- as.matrix(as.matrix(as.matrix(data_i)[,vars]) %*% curcoefs) + 
      #icoefs[1]*(data_i[,vars[1]])^2 + icoefs[2]*(data_i[,vars[2]])^2 +
      cbind(rnorm(nrow(as.matrix(data_i))))
